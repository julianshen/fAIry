...
export const READER_JS = `(() => {
  try {
    ...
    var ptext = function (el) { var n = 0; var ps = el.querySelectorAll('p'); for (var i = 0; i < ps.length; i++) n += (ps[i].textContent || '').length; return n; };
    ...
    if (!root) return null;
    var lines = (root.innerText || '').slice(0, MAX * 2).split('\\n');
    var kept = [];
    ...
    var excerpt = (attr('meta[name="description"]', 'content') || attr('meta[property="og:description"]', 'content') || textContent.slice(0, 200)).slice(0, 1000);
